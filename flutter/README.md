...
- iOS は Swift Package Manager（SPM）経路でビルドします。CocoaPods は不要です
...
### 2. iOS 向け: SPM を有効化（初回のみ）

```bash
flutter config --enable-swift-package-manager
```

このリポジトリには `ios/Podfile` を置いていません。`flutter run` 実行時に SPM 統合が自動で追加されます（`http` / `url_launcher` / `app_links` はいずれも SPM 対応済み）。
